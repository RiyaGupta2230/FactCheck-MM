#!/usr/bin/env python3
"""
BART-Based Paraphrase Generation Model

Implements BartForConditionalGeneration for paraphrase generation with flexibility
to switch between T5 and BART architectures via configuration.
"""
...
@dataclass
class BARTParaphraserConfig:
    """Configuration for BART-based paraphraser."""
    ...
    # Task configuration
    task_prefix: str = "Paraphrase this: "
...
class BARTParaphraser(BaseMultimodalModel):
    """
    BART-based paraphrase generation model.
    
    Provides same functionality as T5Paraphraser but using BART architecture,
    allowing easy comparison between encoder-decoder models.
    """
    
    def __init__(self, config: BARTParaphraserConfig):
        """
        Initialize BART paraphraser.
        ...
        self.config = config
        self.logger = get_logger("BARTParaphraser")
        ...
        if hasattr(config, 'from_scratch') and config.from_scratch:
            bart_config = BartConfig.from_pretrained(config.model_name)
        ...
        self.logger.info(f"Initialized BART paraphraser with {sum(p.numel() for p in self.parameters()):,} parameters")
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        Forward pass for BART paraphraser.
        ...
        **generation_kwargs
        ...
        with torch.no_grad():
            ...
            for seq in generated_ids.sequences:
                # Remove input tokens (BART includes input in output)
    ...
    def _get_generation_config(self, strategy: str, **kwargs) -> GenerationConfig:
        ...
        if strategy == "beam_search":
            ...
            config.update({
                'num_beams': kwargs.get('num_beams', self.config.num_beams),
                'num_beam_groups': kwargs.get('num_beam_groups', 2),
    ...
    def from_pretrained(
        cls,
        model_path: str,
        config: Optional[BARTParaphraserConfig] = None
    ) -> 'BARTParaphraser':
        """
        ...
        Returns:
            Loaded BARTParaphraser instance
        """
        ...
        if config is None:
            ...
            if config_path.exists():
                ...
                config = BARTParaphraserConfig(**config_dict)
            else:
                config = BARTParaphraserConfig()
        
        # Create instance
        instance = cls(config)
        
        # Load model state
        instance.model = BartForConditionalGeneration.from_pretrained(model_path)
        instance.tokenizer = BartTokenizer.from_pretrained(model_path)
...
def main():
    """Example usage of BART paraphraser."""
    ...
    strategies = ["beam_search", "nucleus", "diverse_beam_search"]
